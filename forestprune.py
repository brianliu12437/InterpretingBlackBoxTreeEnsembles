...
def prune_polish_l1(difference_array_list,Y,vars_z,learning_rate, K):
    pred_array = []
    for i in range(len(vars_z)):
        if sum(vars_z[i])>0:
            pred_array.append(np.dot(difference_array_list[i],vars_z[i])*learning_rate)
    ...
    pred_array = np.transpose(pred_array) 
    _, coefs, _ = lasso_path(pred_array, Y)
    cbest = None
    for c in coefs.T:
        if sum(c!=0) > K:
            break
    ...
    A1 = pred_array[:,np.where(cbest!=0)[0]] 
    ridge = 0.0001
    alpha = np.dot((np.dot(np.linalg.inv(np.dot(A1.T,A1) + np.diag(np.repeat(ridge,len(A1[0])))),A1.T)),Y)
    coef_best = np.zeros(len(cbest))
    coef_best[np.where(cbest!=0)[0]] = alpha
    return coef_best
